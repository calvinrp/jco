...
program.command('componentize')
  ...
  .option('--engine <path>', 'Pass a custom engine (SpiderMonkey/StarlingMonkey) build when componentizing')
...
program.command('transpile')
  ...
  .option('--cache-wasm-compile', 'first `instantiate` call caches the compiled Wasm modules for subsequent `instantiate` calls')
  .addOption(new Option('--static-wasm-source-imports [mode]', 'static import of Wasm module').choices(['proposed-standard-import-source', 'non-standard-import']))
  .option('--esm-imports', 'if `--instantiation` is set, use ESM imports instead of providing an import object')
