...
export async function cliTest(_fixtures) {
  suite("CLI", () => {
    ...
    // Ensure that cached modules are used/generated when the
    // --cache-wasm-compile option is set
    test(`--cache-wasm-compile option`, async () => {
      await cachedComponentize(outFile, [
        "test/fixtures/componentize/source.js",
        "-d",
        "all",
        "-w",
...

      {
        const { stderr } = await exec(
          ...
          "-o",
          outDir,
          // NOTE: cache-wasm-compile only takes effect whne instantiation is specified
          "--instantiation=sync",
          "--cache-wasm-compile",
        );
        strictEqual(stderr, "");
      ...
      const jsSource = new TextDecoder().decode(
        await readFile(join(outDir, "componentize.js")),
      );

      ok(
        jsSource.includes("let cachedmodule0;") &&
          jsSource.includes("let cachedmodule1;") &&
          jsSource.includes("let cachedmodule2;"),
      );
    });

    // Ensure that module imports are generated differently depending on the
    // --static-wasm-source-imports=proposed-standard-import-source flag
    test("--static-wasm-source-imports options", async () => {
      await cachedComponentize(outFile, [
        "test/fixtures/componentize/source.js",
        "-d",
        "all",
...

      {
        const { stderr } = await exec(
          ...
          "-o",
          outDir,
          "--static-wasm-source-imports=proposed-standard-import-source",
        );
        strictEqual(stderr, "");
      }

      let jsSource = new TextDecoder().decode(
        await readFile(join(outDir, "componentize.js")),
      );

      ok(
        jsSource.includes(
          "import source module0 from './componentize.core.wasm';",
        ) &&
          jsSource.includes(
            "import source module1 from './componentize.core2.wasm';",
          ) &&
          jsSource.includes(
            "import source module2 from './componentize.core3.wasm';",
          ),
      );

      {
        const { stderr } = await exec(
          ...
          "-o",
          outDir,
          "--static-wasm-source-imports=non-standard-import",
        );
        strictEqual(stderr, "");
      }

      jsSource = new TextDecoder().decode(
        await readFile(join(outDir, "componentize.js")),
      );

      ok(
        jsSource.includes("import module0 from './componentize.core.wasm';") &&
          jsSource.includes(
            "import module1 from './componentize.core2.wasm';",
          ) &&
          jsSource.includes("import module2 from './componentize.core3.wasm';"),
      );
    });

    // Ensure that ESM imports are generated for the --esm-imports flag
    test("--esm-imports options", async () => {
      await cachedComponentize(outFile, [
        "test/fixtures/componentize/source.js",
        "-d",
        "all",
      ...
      ]);
      {
        const { stderr } = await exec(
          ...
          "-o",
          outDir,
          // NOTE: --es-imports only takes effect whne instantiation is specified
          "--instantiation=sync",
          "--esm-imports",
        );
        strictEqual(stderr, "");
